...
function timeToString(time:number) :string{
    ...
    let stingH = hours + ' ' +declOfNum(hours, ['часа', 'часов', 'часов']);
    let stingM = min + ' ' +declOfNum(min, ['минуты', 'минут', 'минут']);
...
export function Stats(){
    ...
    useEffect(()=>{
        let active = new Date(NOW.getFullYear(), NOW.getMonth(), NOW.getDate()  -7*stats.week);
        setActiveDay( parseDate(active,history.days));
    },[])
    return (
        <Wrapper>
